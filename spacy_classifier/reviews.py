...
class SpacySetup:
    def __init__(self, data: pd.DataFrame, spacy_model:str=None):
        if spacy_model:
    ...
    def zip_dataset(self):
        zipped_data = tuple(zip(self.data['Review Text'].tolist(), self.data['Recommended IND'].tolist()))
        return zipped_data
...
doc_bin = DocBin(docs=model.spacy_docs)
