...
class MarkovChain:
    ...
    def __init__(self, text_object, n=2):
        ...
        self.tokens, self.tokens_distinct = self.text_object.tokens, self.text_object.tokens_distinct
    ...
    def tokens_info(self):
        return self.text_object.tokens_info()
    ...
    def create_transition_matrix(self):
        ...
        S_Matrix = scipy.sparse.coo_matrix((values, (row_ind, col_ind)), shape=(len(self.ngram2ind), len(self.token2ind)))
        return S_Matrix
    ...
    def check_prefix(self, prefix: str):
        ...
        if len(prefix_list) < self.n:
            warnings.warn(f'Prefix is too short, please provide of length: {self.n}. Random ngram used instead')
    ...
    @staticmethod
    def add_weights_temperature(input_weights: float, temperature: float):
    ...
    def return_next_word(self, prefix:str, temperature: float = 1.0) -> str:
    ...
    def generate_sequence(self, prefix: str, k: int, temperature: float = 1.0) -> str:
        prefix = self.check_prefix(prefix)
        sequence = prefix.split(' ')

        for i in range(k):
            next_word = self.return_next_word(prefix=prefix, temperature=temperature)
            sequence.append(next_word)
            prefix = ' '.join(sequence[-self.n:])

        return self.reverse_preprocess(' '.join(sequence))

    def bulk_generate_sequence(self, prefix: str, k: int, samples: int, temperature=1.0):
